...
import { REMARK_TYPES, ORDERS } from "@/lib/mock-data";
...
function RemarksPage() {
  const [order, setOrder] = useState<string>(ORDERS[0]!.number);
  const [type, setType] = useState<string>(REMARK_TYPES[0]!);
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Select value={order} onValueChange={setOrder}>
                ...
                <SelectContent>
                  {ORDERS.slice(0, 12).map((o) => (
                    <SelectItem key={o.id} value={o.number}>{o.number} · {o.customer}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={type} onValueChange={setType}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {REMARK_TYPES.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
        ...
        <div className="surface-panel p-6">
          ...
          <ul className="space-y-3">
            {[
              { o: "ORD261012", t: "QC Hold", m: "Awaiting client colour approval." },
              { o: "ORD261008", t: "Production Handoff", m: "Passed to lamination." },
              { o: "ORD261003", t: "Internal Note", m: "Substrate swap approved." },
            ].map((r, i) => (
