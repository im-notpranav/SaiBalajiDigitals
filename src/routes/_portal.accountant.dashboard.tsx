...
import { inr, ordersByStatus, REVENUE_TREND } from "@/lib/mock-data";
...
function AccountantDashboard() {
  const pendingBilling = ordersByStatus("ready_to_bill").length;
  const awaitingClosure = ordersByStatus("billed").length;
  const fyRevenue = REVENUE_TREND.reduce((s, r) => s + r.revenue, 0);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <KpiCard label="FY Revenue" value={inr(fyRevenue)} icon={TrendingUp} accent="success" delay={0.1} trend={{ value: "+18.2%", positive: true }} />
